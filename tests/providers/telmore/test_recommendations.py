"""Test the Telmore Musik two-method recommendations contract."""
...
def _stub_api(provider: TelmoreMusikProvider) -> Mock:
    ...
    provider.api = api
    provider.auth = Mock()
    provider.recommendations_manager = TelmoreRecommendationsManager(provider)
    return api


@pytest.mark.asyncio
async def test_get_recommendations_returns_rows_without_items(
    provider: TelmoreMusikProvider,
) -> None:
