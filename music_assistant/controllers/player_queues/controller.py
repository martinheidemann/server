...
class PlayerQueuesController(QueueLoaderMixin, PlaybackTrackerMixin, StreamFeederMixin):
    ...
    async def skip(self, queue_id: str, seconds: int = 10) -> None:
        ...
        queue_data = self._queue_data[queue_id]
        # a press within the debounce window composes onto the target its predecessor published:
        # the queue clock is frozen at that target and extrapolating past it, so it is not a base.
        # otherwise the stored elapsed_time is only refreshed once a second, so a skip reading it
        # directly would land short by however long ago the queue was last updated.
        pending = queue_data.pending_skip
        base = (
            pending.target
            if pending is not None and pending.queue_item_id == current_item.queue_item_id
            else queue.corrected_elapsed_time
        )
        target = self._clamp_skip_target(base + seconds, current_item.duration)
        queue_data.pending_skip = PendingSkip(current_item.queue_item_id, target)

        # freeze queue updates from the player, so the position published below survives the
        # debounce window instead of being overwritten by the poll (as next/previous do)
        self._set_transitioning(queue_id, True)
        queue.elapsed_time = target
        queue.elapsed_time_last_updated = time.time()
        self.signal_update(queue_id)

        # shares its task_id with next/previous/play_index, so any of those cancels a pending
        # skip and a skip cancels a pending track change
        self.mass.call_later(
            SKIP_DEBOUNCE_DELAY,
    ...
    async def _apply_pending_skip(self, queue_id: str) -> None:
        ...
        if (pending := queue_data.pending_skip) is None:
            return
        queue = queue_data.queue
        # the burst is settled, so drop the target now: a press arriving during the rebuild
        # composes onto the fresh clock play_index anchors at exactly this target
